...
const ServiceIcon = ({ type, className = "w-16 h-16" }: ServiceIconProps) => {
  const icons = {
    automation: (
  ...
  return (
    <div className="flex justify-center items-center p-4 bg-gradient-to-br from-brand-gold/10 to-brand-gold/20 rounded-2xl group-hover:from-brand-gold/20 group-hover:to-brand-gold/30 transition-all duration-500 group-hover:scale-110">
      {icons[type]}
    </div>
  );
};
